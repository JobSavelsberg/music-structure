<template>
  <div>
    <div v-if="hasStructure">
      <div
        class="structure"
        v-for="structure in track.structures"
        :key="structure.name"
      >
        <v-row class="mb-0 pt-2">
          <v-btn
            v-if="!structure.separateByGroup"
            icon
            small
            @click="clickStructureUnfold(structure)"
          >
            <v-icon>
              {{
...
export default {
  ...
  methods: {
    ...
    sectionVerticalPosition(structure, section) {
      ...
      if (structure.verticalPosition) {
        switch (structure.vertLayout) {
          ...
          case "linear":
            return (
              section.mdsFeature * this.blockHeight * this.verticalPositionScale
            );
          default:
          case "circular":
            return (
              section.colorAngle * this.blockHeight * this.verticalPositionScale
            );
        }
    ...
    hoverSection(event, section, structure) {
      this.tooltipTimeout = setTimeout(() => {
        this.toolTipText = `
                ...
                [${parseFloat(section.start).toFixed(2)}, ${parseFloat(
          section.end
        ).toFixed(2)}] <br /> 
                Duration: ${parseFloat(section.end - section.start).toFixed(
                  2
                )} <br /> 
                ColorAngle: ${parseFloat(section.colorAngle).toFixed(2)} <br />
                MDSFeature: ${parseFloat(section.mdsFeature).toFixed(2)} <br />
                NormScore: ${parseFloat(section.normalizedScore).toFixed(
                  2
                )} <br /> 
                NormCoverage: ${parseFloat(section.normalizedCoverage).toFixed(
                  2
                )} <br /> 
        ...
        setTimeout(() => {
          ...
          tooltips.forEach((tool) => {
            tool.style.left =
              event.pageX - event.offsetX + section.start * this.scale + "px";
            tool.style.top =
              event.pageY -
              event.offsetY +
              (structure.separateByGroup ? section.groupID + 1 : 1) *
                this.blockHeight +
              "px";
          });
        }, 0);
    ...
    clickSection(event, section) {
      if (event.shiftKey) {
        const startInSamples =
          section.start / this.track.features.sampleDuration;
    ...
    generatePointsForShapedSection(structure, section) {
      ...
      const endInSamples = Math.floor(
        section.end / this.track.features.sampleDuration
      );
      ...
      const startYHalfNeg = Math.round(
        yMid -
          0.8 *
            this.averageLoudness[endOffsetInSamples] *
            halfHeight *
            heightFactor
      );
      const startYPNeg = Math.round(
        yMid -
          this.averageLoudness[endOffsetInSamples] * halfHeight * heightFactor
      );
      let points = `M ${x} ${Math.round(
        yMid
      )} L ${x} ${startYHalfNeg} Q ${x} ${startYPNeg}`;

      for (let i = startInSamples; i < endInSamples; i++) {
        if (i === startInSamples + endOffsetInSamples) {
          const pointX = Math.round(
            x + ((i - startInSamples) / durationInSamples) * width
          );
          const pointY = Math.round(
            yMid - this.averageLoudness[i] * halfHeight * heightFactor
          );
